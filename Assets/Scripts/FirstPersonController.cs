...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float handRotation = hands.localRotation.eulerAngles.x;
        if (handRotation <= 80 && jumping || handRotation > 300 && jumping)
        {
            hands.Rotate(new Vector3(500f, 0, 0)* Time.deltaTime);
        }
        else
        {
            jumping = false;
        }
        if (!jumping && (handRotation >= 0 && handRotation < 90))
        {
            hands.Rotate(new Vector3(-500f, 0, 0) * Time.deltaTime);
        }
        ...
        {
            leftHand.position = Vector3.MoveTowards(leftHand.position, leftHandEnd.position, 5f * Time.deltaTime);
            rightHand.position = Vector3.MoveTowards(rightHand.position, rightHandEnd.position, 5f * Time.deltaTime);
        }
        else
        {
            leftHand.position = Vector3.MoveTowards(leftHand.position, leftHandOrigin.position, 5f * Time.deltaTime);
            rightHand.position = Vector3.MoveTowards(rightHand.position, rightHandOrigin.position, 5f * Time.deltaTime);
        }
        #endregion
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.transform.CompareTag("Checkpoint"))
        {
            ...
            this.startPosition = this.transform.position + (this.transform.forward * 2);
        }
    }
}
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("Hands setup", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();

        fpc.hands = (Transform)EditorGUILayout.ObjectField(new GUIContent("Hands", "Hier stoppen we de handen in."), fpc.hands, typeof(Transform), true);
        fpc.handsOrigin = (Transform)EditorGUILayout.ObjectField(new GUIContent("HandsOrigins", "Hier stoppen we de HandsOrigin in."), fpc.handsOrigin, typeof(Transform), true);
        fpc.rightHand = (Transform)EditorGUILayout.ObjectField(new GUIContent("Right Hand", "Hier stoppen we de rechter hand in."), fpc.rightHand, typeof(Transform), true);
        fpc.rightHandEnd = (Transform)EditorGUILayout.ObjectField(new GUIContent("Right Hand End", "Hier stoppen we de rechter hand in."), fpc.rightHandEnd, typeof(Transform), true);
        fpc.rightHandOrigin = (Transform)EditorGUILayout.ObjectField(new GUIContent("Right Hand Origin", "Hier stoppen we de rechter hand in."), fpc.rightHandOrigin, typeof(Transform), true);
        fpc.leftHand = (Transform)EditorGUILayout.ObjectField(new GUIContent("Left Hand", "Hier stoppen we de linker hand in."), fpc.leftHand, typeof(Transform), true);
        fpc.leftHandEnd = (Transform)EditorGUILayout.ObjectField(new GUIContent("Left Hand End", "Hier stoppen we de linker hand in."), fpc.leftHandEnd, typeof(Transform), true);
        fpc.leftHandOrigin = (Transform)EditorGUILayout.ObjectField(new GUIContent("Left Hand Origin", "Hier stoppen we de linker hand in."), fpc.leftHandOrigin, typeof(Transform), true);
